...
void EVMHost::reset()
{
	...
	// EIP-NTT precompiles at 0x0101-0x0104.
	for (unsigned nttAddress: {0x0101u, 0x0102u, 0x0103u, 0x0104u})
	{
		evmc::address address{nttAddress};
		accounts[address].balance = evmc::uint256be{1};
		accounts[address].codehash = 0xc5d2460186f7233c927e7db2dcc703c0e500b653ca82273b7bfad8045d85a470_bytes32;
	}
}
...
evmc::Result EVMHost::call(evmc_message const& _message) noexcept
{
	...
	else if (_message.recipient == 0x0000000000000000000000000000000000000101_address)
		return precompileNTTForward(_message);
	else if (_message.recipient == 0x0000000000000000000000000000000000000102_address)
		return precompileNTTInverse(_message);
	else if (_message.recipient == 0x0000000000000000000000000000000000000103_address)
		return precompileNTTVecMulMod(_message);
	else if (_message.recipient == 0x0000000000000000000000000000000000000104_address)
		return precompileNTTVecAddMod(_message);
...
// ============================================================
// EIP-NTT precompile helpers (file-local)
// ============================================================
namespace
{

constexpr int64_t NTT_FW_GAS = 600;
constexpr int64_t NTT_INV_GAS = 600;
constexpr size_t NTT_HEADER_SIZE = 64;    // q (32 bytes) + n (32 bytes)
constexpr size_t NTT_ELEMENT_SIZE = 32;   // each coefficient is uint256 big-endian
constexpr uint64_t NTT_MAX_DEGREE = 65536;

// Parse q (modulus) and n (degree) from the first 64 bytes of input.
bool nttParseHeader(uint8_t const* data, size_t size, bigint& q, uint64_t& n)
{
	if (size < NTT_HEADER_SIZE)
		return false;
	q = fromBigEndian<bigint>(bytes(data, data + 32));
	if (q <= 0)
		return false;
	bigint nBig = fromBigEndian<bigint>(bytes(data + 32, data + 64));
	if (nBig < 2 || nBig > NTT_MAX_DEGREE)
		return false;
	n = static_cast<uint64_t>(nBig);
	if ((n & (n - 1)) != 0) // must be a power of 2
		return false;
	return true;
}

// Parse n big-endian 32-byte coefficients from input at the given offset.
// Validates each coefficient is strictly less than q.
bool nttParseVector(uint8_t const* data, size_t size, size_t offset, uint64_t n, bigint const& q, std::vector<bigint>& vec)
{
	size_t end = offset + n * NTT_ELEMENT_SIZE;
	if (end > size)
		return false;
	vec.resize(n);
	for (uint64_t i = 0; i < n; i++)
	{
		size_t start = offset + i * NTT_ELEMENT_SIZE;
		vec[i] = fromBigEndian<bigint>(bytes(data + start, data + start + NTT_ELEMENT_SIZE));
		if (vec[i] >= q)
			return false;
	}
	return true;
...
// Encode a vector of bigints as n*32 big-endian bytes.
bytes nttEncodeOutput(std::vector<bigint> const& vec)
{
	bytes out(vec.size() * NTT_ELEMENT_SIZE, 0);
	for (size_t i = 0; i < vec.size(); i++)
	{
		bytes elem(NTT_ELEMENT_SIZE, 0);
		toBigEndian(vec[i], elem);
		std::copy(elem.begin(), elem.end(), out.begin() + i * NTT_ELEMENT_SIZE);
	}
	return out;
}

// Returns the smallest power of 2 that is >= ceil(log2(q)).
uint64_t nttComputeK(bigint const& q)
{
	if (q <= 1)
		return 1;
	unsigned bitLen = static_cast<unsigned>(boost::multiprecision::msb(q)) + 1;
	uint64_t k = 1;
	while (k < bitLen)
		k <<= 1;
	return k;
...
// Iterative extended GCD: computes the modular inverse of a modulo m.
// Returns false if the inverse does not exist (gcd != 1).
bool nttModInverse(bigint const& a, bigint const& m, bigint& result)
{
...
// NTT_FW (0x0101): forward NTT using Cooley-Tukey butterfly.
// Input: q(32) | n(32) | a[n](n*32) | psiRev[n](n*32)
// Output: a[n](n*32) in bit-reversed order
evmc::Result EVMHost::precompileNTTForward(evmc_message const& _message) noexcept
{
	static bytes output;

	bigint q;
	uint64_t n;
	if (!nttParseHeader(_message.input_data, _message.input_size, q, n))
		return resultWithFailure();
	if (_message.input_size != NTT_HEADER_SIZE + 2 * n * NTT_ELEMENT_SIZE)
		return resultWithFailure();

	std::vector<bigint> a, psiRev;
	if (!nttParseVector(_message.input_data, _message.input_size, NTT_HEADER_SIZE, n, q, a))
		return resultWithFailure();
	if (!nttParseVector(_message.input_data, _message.input_size, NTT_HEADER_SIZE + n * NTT_ELEMENT_SIZE, n, q, psiRev))
		return resultWithFailure();

	bigint U, V;
	uint64_t t = n;
	for (uint64_t m = 1; m < n; m *= 2)
	{
		t /= 2;
		for (uint64_t i = 0; i < m; i++)
		{
			bigint const& S = psiRev[m + i];
			uint64_t j1 = 2 * i * t;
	...
	output = nttEncodeOutput(a);
	return resultWithGas(_message.gas, NTT_FW_GAS, output);
}

// NTT_INV (0x0102): inverse NTT using Gentleman-Sande butterfly.
// Input: q(32) | n(32) | a[n](n*32) | psiInvRev[n](n*32)
// Output: a[n](n*32) in standard order
evmc::Result EVMHost::precompileNTTInverse(evmc_message const& _message) noexcept
{
	static bytes output;
	...
	std::vector<bigint> a, psiInvRev;
	if (!nttParseVector(_message.input_data, _message.input_size, NTT_HEADER_SIZE, n, q, a))
		return resultWithFailure();
	if (!nttParseVector(_message.input_data, _message.input_size, NTT_HEADER_SIZE + n * NTT_ELEMENT_SIZE, n, q, psiInvRev))
		return resultWithFailure();

	bigint U, V;
	uint64_t t = 1;
	for (uint64_t m = n; m > 1; m /= 2)
	{
		uint64_t j1 = 0;
		uint64_t h = m / 2;
		for (uint64_t i = 0; i < h; i++)
		{
			bigint const& S = psiInvRev[h + i];
			for (uint64_t j = j1; j < j1 + t; j++)
			{
				U = a[j];
				V = a[j + t];
	...
	// Final scaling: a[j] *= n^{-1} mod q
	bigint nInv;
	if (!nttModInverse(bigint(n), q, nInv))
		return resultWithFailure();
	for (uint64_t j = 0; j < n; j++)
		a[j] = (a[j] * nInv) % q;

	output = nttEncodeOutput(a);
	return resultWithGas(_message.gas, NTT_INV_GAS, output);
}

// NTT_VECMULMOD (0x0103): element-wise modular multiplication.
// Input: q(32) | n(32) | a[n](n*32) | b[n](n*32)
// Output: (a[i]*b[i] mod q)[n](n*32)
evmc::Result EVMHost::precompileNTTVecMulMod(evmc_message const& _message) noexcept
{
	static bytes output;
	...
	if (!nttParseVector(_message.input_data, _message.input_size, NTT_HEADER_SIZE, n, q, a))
		return resultWithFailure();
	if (!nttParseVector(_message.input_data, _message.input_size, NTT_HEADER_SIZE + n * NTT_ELEMENT_SIZE, n, q, b))
		return resultWithFailure();

	std::vector<bigint> result(n);
	for (uint64_t i = 0; i < n; i++)
		result[i] = (a[i] * b[i]) % q;

	uint64_t k = nttComputeK(q);
	unsigned log2n = 0;
	for (uint64_t tmp = n; tmp > 1; tmp >>= 1)
		log2n++;
	int64_t gas = static_cast<int64_t>(k * log2n / 8);
	if (gas == 0)
		gas = 1;

	output = nttEncodeOutput(result);
	return resultWithGas(_message.gas, gas, output);
}

// NTT_VECADDMOD (0x0104): element-wise modular addition.
// Input: q(32) | n(32) | a[n](n*32) | b[n](n*32)
// Output: (a[i]+b[i] mod q)[n](n*32)
evmc::Result EVMHost::precompileNTTVecAddMod(evmc_message const& _message) noexcept
{
	static bytes output;
	...
	for (uint64_t tmp = n; tmp > 1; tmp >>= 1)
		log2n++;
	int64_t gas = static_cast<int64_t>(k * log2n / 32);
	if (gas == 0)
		gas = 1;
